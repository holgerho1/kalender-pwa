...
const CONFIG = {
    ...
    FIELDS: {
        ...
        'Diesel': { label: 'Medium' }, 'Ventil': { label: 'Heberschutzventil' },
        'Zustand': { label: 'Allgemeinzustand' }, 'Körper': { label: 'Ventilkörper' },
        'Inhalt': { unit: 'Liter' }, 'Durch': { label: 'Durchmesser', unit: 'cm' },
...
window.dateCh = () => { localStorage.setItem('form_Datum', document.getElementById('dateIn').value); updateUI(); setTimeout(window.closeM, 150); };
...
async function init() {
    try {
        ...
        const savedDate = localStorage.getItem('form_Datum');
        document.getElementById('dateIn').value = savedDate || new Date().toISOString().split('T')[0];
...
function setupFields() {
    detectedTags.forEach(tag => {
        if (tag === 'Datum') return;
        let containerId = 'fExtra';
        if (CONFIG.GROUPS.addr.includes(tag)) containerId = 'fAddr';
        else if (CONFIG.GROUPS.anlage.includes(tag)) containerId = 'fAnlage';
        else if (CONFIG.GROUPS.mech.includes(tag)) containerId = 'fMech';
        else if (CONFIG.GROUPS.mess.includes(tag)) containerId = 'fMess';
        else if (CONFIG.GROUPS.res.includes(tag)) containerId = 'fResult';
        
        createInputField(tag, document.getElementById(containerId));
    });
    ...
    if (detectedTags.has('Mindest')) {
        ...
        document.getElementById('fResult').prepend(row);
    }
}
...
function createInputField(tag, container) {
    ...
    group.innerHTML = `
        <label>${conf.label || tag}</label>
        <div class="${conf.unit ? 'input-with-unit' : ''}">
            <input id="i_${tag}" ${conf.readOnly ? 'readonly' : ''}>
            ${conf.unit ? `<span class="unit-label">${conf.unit}</span>` : ''}
        </div>`;
    ...
    document.querySelector(`[onclick*="${container.id.replace('f', '').toLowerCase()}"]`)?.style.setProperty('display', 'block');
}
...
function calculateReference() {
    ...
    const fields = [document.getElementById('i_Mindest'), document.getElementById('i_Mindest_Ref')];
    fields.forEach(f => { if(f) f.value = res; });
    localStorage.setItem('form_Mindest', res);
}

function updateUI() {
    const buildPrev = (tags, labels) => tags.map(t => {
        const v = document.getElementById('i_'+t)?.value;
        if (!v) return null;
        return labels ? `${CONFIG.FIELDS[t]?.label || t}: ${v}${CONFIG.FIELDS[t]?.unit ? ' '+CONFIG.FIELDS[t].unit : ''}` : v;
    }).filter(x => x).join('\n');

    document.getElementById('previewTextAddr').textContent = buildPrev(CONFIG.GROUPS.addr, false) || "---";
    document.getElementById('previewTextAnlage').textContent = buildPrev(CONFIG.GROUPS.anlage, true) || "---";
    document.getElementById('previewTextMechanik').textContent = buildPrev(CONFIG.GROUPS.mech, true) || "---";
    document.getElementById('previewTextMess').textContent = buildPrev(CONFIG.GROUPS.mess, true) || "---";
    document.getElementById('previewTextResult').textContent = buildPrev([...(detectedTags.has('Mindest')?['Mindest']:[]) ,...CONFIG.GROUPS.res], true) || "---";
...
document.getElementById('upBtn').onclick = async () => {
    ...
    const tok = document.getElementById('ghTok').value;
    ...
    reader.onload = async () => {
        ...
        try {
            ...
            const get = await fetch(url, { headers: { 'Authorization': `token ${tok}` } });
            const d = await get.json();
            const res = await fetch(url, {
                method: 'PUT', headers: { 'Authorization': `token ${tok}`, 'Content-Type': 'application/json' },
                body: JSON.stringify({ message: "Update Template", content: b64, sha: d.sha })
            });
            alert(res.ok ? "Erfolgreich!" : "Fehler!");
