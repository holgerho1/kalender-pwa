...
function renderSteuerung(container) {
  ...
  const pdfBtn = btn("PDF Export", "picture_as_pdf", "nav-pdf", async () => {
    // 1. Alle Mitarbeiterdaten laden (Flags + DB-Text für Z2)
    const mitarbeiter = await ladeMitarbeiterId();

    // 2. Aktuellen Text aus Datenbox 2 (Z1) einsammeln, falls vorhanden
    ...
    // 4. PDF mit den gesammelten Kontext-Daten exportieren
    exportierePdf(holeGefilterteTermine(getKWZeitraum(getKwOffset())), mitarbeiter);
  }, SECONDARY, "#000");
  pdfBtn.style.gridColumn = "span 2";
  sDiv.appendChild(pdfBtn);
...
async function ladeMitarbeiterId() {
  const pathParts = window.location.pathname.split("/");
  const kuerzel = pathParts.pop() || pathParts.pop();
  // Z2 und das Textfeld 'infotext' aus der DB abfragen
  const { data, error } = await supa.from("mitarbeiter").select("id, name, Z1, Z2, infotext").eq("kuerzel", kuerzel).single();
  return error ? null : data;
}
