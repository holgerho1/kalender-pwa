...
function erstelleTerminKarte(event) {
  ...
  block.style = "margin-bottom: 1rem; padding: 1rem; background: #fff; border-left: 4px solid #0077cc; border-radius: 6px; box-shadow: 0 0 4px rgba(0,0,0,0.1); box-sizing: border-box; width: 100%;";
  ...
  block.innerHTML = `
    ...
    <textarea class="titel-input" rows="2" style="width:100%; margin-top:0.5rem; border:1px solid #ccc; border-radius:4px; padding:4px; box-sizing: border-box;"></textarea>
    
    <div style="display: flex; gap: 8px; margin-top: 0.5rem; width: 100%;">
      <input type="text" class="stunden-input" data-field="arbeit" value="${event.arbeit || ""}" placeholder="Arbeit" style="width: 33%; min-width: 0; border:1px solid #ccc; border-radius:4px; padding:4px; box-sizing: border-box;">
      <input type="text" class="stunden-input" data-field="fahr" value="${event.fahr || ""}" placeholder="Fahr" style="width: 33%; min-width: 0; border:1px solid #ccc; border-radius:4px; padding:4px; box-sizing: border-box;">
      <input type="text" class="stunden-input" data-field="über" value="${event.über || ""}" placeholder="Über" style="width: 33%; min-width: 0; border:1px solid #ccc; border-radius:4px; padding:4px; box-sizing: border-box;">
    </div>
    
    <textarea class="desc-input" rows="3" style="width:100%; margin-top:0.5rem; border:1px solid #ccc; border-radius:4px; padding:4px; box-sizing: border-box;"></textarea>
    <textarea class="mat-input" rows="3" placeholder="Material" style="width:100%; margin-top:0.5rem; border:1px solid #ccc; border-radius:4px; padding:4px; box-sizing: border-box;"></textarea>
    <textarea class="mit-input" rows="2" style="width:100%; margin-top:0.5rem; border:1px solid #ccc; border-radius:4px; padding:4px; box-sizing: border-box;"></textarea>
  `;

  // Werte in Textareas setzen (um HTML-Injektion zu vermeiden)
  ...
  loeschen.style = "margin-top: 10px; width: 100%; padding: 8px; background: #fee; border: 1px solid #fcc; border-radius: 4px; cursor: pointer;";
  loeschen.onclick = () => {
...
function renderDatenbox1(container, stats, { montag }, mitarbeiterId) {
  ...
  box.style = "margin-top: 1rem; padding: 1rem; background: #fff; border-radius: 6px; box-shadow: 0 0 4px rgba(0,0,0,0.1); box-sizing: border-box; width: 100%;";
  box.innerHTML = `
...
async function renderDatenbox2(container, stats, { montag }, mitarbeiterId) {
  ...
  if (!daten || daten.length === 0) {
    box.innerHTML = `<strong>Letzter Eintrag</strong><br><br>Keine Daten gefunden.<br><br><button id="speichernBtn" style="width:100%; padding:10px;">Speichern</button>`;
  } else {
    ...
    box.innerHTML = `
      <style>.row { display: grid; grid-template-columns: 1fr auto 80px; align-items: center; margin-bottom: 6px; gap: 10px; } .row input { width: 80px; text-align: right; box-sizing: border-box; }</style>
      <strong>${gleicheKW ? `Daten aus KW ${eintrag.KW}/${eintrag.JAHR}` : `Vorschlag (KW ${eintrag.KW} + aktuelle KW)`}</strong><br><br>
      <div class="row"><span>Urlaub:</span><span>${eintrag.URLAUB ?? 0} =</span><input id="urlaubWert" type="number" value="${eintrag.URLAUB ?? 0}"></div>
      <div class="row"><span>Urlaub gen.:</span><span>${eintrag.URLAUBgen ?? 0} ${!gleicheKW ? `+ ${stats.urlaub}` : ""} =</span><input id="urlaubErgebnis" type="number" value="${v.uG}"></div>
      ...
      <div class="row"><span>Bereit:</span><span>${eintrag.BEREIT ?? 0} ${!gleicheKW ? `+ ${stats.bereit}` : ""} =</span><input id="bereitErgebnis" type="number" value="${v.b}"></div>
      ...
      Text:<br><textarea id="textBearbeiten" style="width:100%; height:60px; margin-top:10px; box-sizing: border-box;">${eintrag.feld1 ?? ""}</textarea>
      <br><br><button id="speichernBtn" style="width:100%; padding:10px;">Speichern</button>
    `;
  }
...
function renderSteuerung(container) {
  ...
  sDiv.style = "margin-top: 1rem; display: flex; flex-wrap: wrap; gap: 8px; justify-content: center;";
  
  const btn = (t, f) => {
    const b = document.createElement("button"); 
    b.textContent = t; 
    b.style = "padding: 10px; border-radius: 4px; border: 1px solid #ccc; background: #fff; cursor: pointer; flex: 1; min-width: 100px;";
    b.onclick = f; 
    return b;
  };
  
  sDiv.appendChild(btn("🧹 Laden", neuLaden));
  ...
  pdfBtn.style.backgroundColor = "#eee";
