...
datenBox2.innerHTML = `
  ...
  \${
    \`
      ...
      Urlaub genommen: \${gleicheKW ? (eintrag.URLAUBgen ?? 0) : (eintrag.URLAUBgen ?? 0) + urlaubCount}
      Krank: \${gleicheKW ? (eintrag.KRANK ?? 0) : (eintrag.KRANK ?? 0) + krankCount}
      Überstunden: \${
        gleicheKW
          ? (parseFloat(eintrag["ÜBER"] ?? 0) || 0).toFixed(2)
          : (
      ...
      Bereitschaft: \${gleicheKW ? (eintrag.BEREIT ?? 0) : (eintrag.BEREIT ?? 0) + bereitschaftCount}
      \${eintrag.feld1 ?? ""}
    \`.replace(/\\s+/g, " ")
  }
`;
